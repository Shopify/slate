...
const messages = {
    ...
    gutil.log(
      ...
      gutil.colors.yellow(
        `${typeof errMsg === 'string'
          ? errMsg
          : 'File(s) failed to upload to store. See log notes above.'}`
      )
    );
  },
  ...
  configChange: () => {
    return (
      'Changes to ThemeKit Config Detected: You may need to quit <slate watch>' +
  ...
  configError: () => {
    gutil.log(
      'File missing:',
      gutil.colors.yellow(
        '`config.yml` does not exist. You need to add a config file before you can make changes to your Shopify store.'
      )
    );
  },
